...
final class ChangePassView: UIView {
    ...
    private let sendButton = ReusableAuthButton(title: "CHANGE PASSWORD", target: ChangePassView.self, action: #selector(changeButtonTapped))
    private let showPassOneButton = UIButton(target: ChangePassView.self, action: #selector(showPassOneTapped))
    private let showPassTwoButton = UIButton(target: ChangePassView.self, action: #selector(showPassTwoTapped))
